...
/*
void freeHelperFunction(TfIdfList tree, FILE *fp) {
    // Base case, if the tree is empty
    if (tree == NULL) {
        return;
    }
    ...
    // We already have a sorted tree so we navigate to the left until it is NULL to print lowest to highest
    freeHelperFunction(tree->left, fp);

    FileList curr = tree->fileList;
    // Printing out the file linked list which is already sorted alphabetically
    while (curr != NULL) {
        curr->filename=null; //nullptr
        curr->tf=null;      //nullptr
        curr = curr->next;
    }
    freeHelperFunction(tree->right, fp);
}
*/
/////////////////////////////////////////////////////////////////////////////////
...
void freeInvertedIndex(InvertedIndexBST tree) {
  ...
  while (curr != NULL) {
    ...
    fprintf(fp,"(%lf) ",curr->tf);
    curr = curr->next;
    free(tmp->filename);  // Note: only do this if filename was allocated dynamically
    free(tmp); // Note: only do this if FileList was allocated dynamically
  }

  fprintf(fp,"\\n");
  freeInvertedIndex(tree->right);
  free(tree); // Note: only do this if tree was allocated dynamically
}
